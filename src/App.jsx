...
import { Search, LogOut, Eye, Plus, Upload, Download, Users as UsersIcon, FileText, DollarSign, Calendar, TrendingUp, MapPin, User, Trash2, Phone, Mail } from 'lucide-react'
...
export default function App() {
  ...
  useEffect(() => {
    ...
    style.innerHTML = `* { -webkit-user-select: none !important; -moz-user-select: none !important; -ms-user-select: none !important; user-select: none !important; } input, textarea { -webkit-user-select: text !important; -moz-user-select: text !important; -ms-user-select: text !important; user-select: text !important; }`
    ...
    const handleScreenshot = async () => {
      ...
      if (screenshotAttempts === 1) {
        ...
        await supabase.from('notes').insert([{ lead_id: selectedLead?.id || null, text: `🚨 SECURITY ALERT: User ${user.name} (${user.username}) attempted ${screenshotAttempts} screenshots at ${new Date().toLocaleString()}`, author: 'SYSTEM', created_at: new Date().toISOString() }])
      }
    }
  ...
  // ── UNLIMITED LEADS: fetch all rows in batches of 1000 ──
  const loadData = async () => {
    setLoading(true)
    try {
      // Fetch all leads with pagination to bypass Supabase 1000-row limit
      let allLeads = []
      let from = 0
  ...
  const loadContacts = async (leadId) => {
    try {
      const { data, error } = await supabase.from('contacts').select('*').eq('lead_id', String(leadId)).order('created_at', { ascending: false })
  ...
  const saveContact = async (formData, editingId) => {
    try {
      ...
      if (!formData.first_name?.trim() && !formData.last_name?.trim()) throw new Error('Please enter at least a first or last name')
      const contactData = {
      ...
      let result
      if (editingId) {
        result = await supabase.from('contacts').update(contactData).eq('id', editingId).select()
      } else {
        result = await supabase.from('contacts').insert([{ ...contactData, created_at: new Date().toISOString() }]).select()
      }
      if (result.error) throw new Error(result.error.message)
      const updatedContacts = await loadContacts(selectedLead.id)
      setLeadContacts(updatedContacts)
      alert(editingId ? '✅ Contact updated!' : '✅ Contact saved!')
      return result.data
    } catch (err) {
  ...
  const login = async (e) => {
    ...
    if (data) { setUser(data); loadData(); setView('dashboard') }
  ...
  const updateStatus = async (id, status) => {
    try {
      const { error } = await supabase.from('leads').update({ status, last_modified: new Date().toISOString() }).eq('id', id)
      if (error) throw error
  ...
  const assign = async (id, userId) => {
    try {
      const { error } = await supabase.from('leads').update({ assigned_to: userId, last_modified: new Date().toISOString() }).eq('id', id)
      if (error) throw error
  ...
  const addNote = async () => {
    ...
    try {
      const { error } = await supabase.from('notes').insert([{ lead_id: selectedLead.id, text: note, author: user.name, created_at: new Date().toISOString() }])
      if (error) throw error
      setNote('')
  ...
  const updateNote = async (noteId, newText) => {
    ...
    try {
      const { error } = await supabase.from('notes').update({ text: newText }).eq('id', noteId)
      if (error) throw error
      const notes = await loadNotes(selectedLead.id)
      setSelectedLead({ ...selectedLead, notes })
      setEditingNote(null)
  ...
  const uploadLeads = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from('leads').upsert(formatted)
      if (error) throw error
      loadData()
      setUploadText('')
      alert(`Uploaded ${data.length} leads!`)
    } catch (err) { alert('Invalid JSON or upload failed') }
  }

  const exportData = () => {
    const blob = new Blob([JSON.stringify(sortedFiltered, null, 2)], { type: 'application/json' })
  ...
  // ── FIXED TYPE FILTER: case-insensitive, handles FORECLOSURE / TAXDEED / TAX DEED ──
  const getLeadTypeNormalized = (lt) => (lt || '').toUpperCase().replace('_', ' ').trim()

  const filtered = leads.filter(l => {
    if (user?.role !== 'admin' && l.assigned_to !== user?.id) return false
    if (filters.status !== 'all' && l.status !== filters.status) return false
    if (filters.type !== 'all') {
      const lt = getLeadTypeNormalized(l.lead_type)
  ...
  const stats = {
    ...
    surplus: leads.filter(l => getLeadTypeNormalized(l.lead_type) === 'SURPLUS').length,
    future: leads.filter(l => getLeadTypeNormalized(l.lead_type) === 'FUTURE AUCTION').length,
  ...
  // ── HELPERS ──
  const getTypeBadge = (lead_type) => {
    const lt = getLeadTypeNormalized(lead_type)
    if (lt === 'SURPLUS') return { label: 'Surp', cls: 'bg-emerald-500/20 text-emerald-400' }
    if (lt === 'FUTURE AUCTION') return { label: 'Futr', cls: 'bg-blue-500/20 text-blue-400' }
    if (lt === 'FORECLOSURE') return { label: 'FC', cls: 'bg-orange-500/20 text-orange-400' }
    if (lt === 'TAXDEED' || lt === 'TAX DEED') return { label: 'TD', cls: 'bg-purple-500/20 text-purple-400' }
    return { label: lead_type || '?', cls: 'bg-slate-500/20 text-slate-400' }
  }
  ...
  const formatSurplus = (surplus) => {
    if (!surplus) return null
    const raw = parseFloat(String(surplus).replace(/[$,]/g, ''))
    if (isNaN(raw)) return surplus
    const rounded = Math.round(raw * 100) / 100
    const formatted = '$' + Math.abs(rounded).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })
    return { value: rounded, formatted, negative: rounded < 0 }
  }

  if (loading) return (
    <div className="min-h-screen bg-slate-900 flex items-center justify-center">
      <div className="text-center">
        <div className="text-amber-400 text-xl mb-2">Loading leads...</div>
        <div className="text-slate-400 text-sm">Fetching all records from database</div>
      </div>
    </div>
  )
  ...
  if (view === 'admin') return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
      ...
      <div className="max-w-7xl mx-auto px-6 py-8 space-y-6">
        ...
        <div className="bg-slate-800/50 border border-slate-700/50 rounded-xl p-6">
          <h2 className="text-xl font-bold text-white mb-4 flex items-center gap-2"><UsersIcon className="w-5 h-5 text-amber-400" /> Users ({users.length})</h2>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-slate-800/50 border border-slate-700/50 rounded-xl p-6">
              <div className="flex items-start justify-between mb-4">
                <div>
                  {/* Show both lead_type badge AND auction type (Foreclosure/Tax Deed) */}
                  <div className="flex gap-2 mb-3">
                    <span className={`inline-block px-3 py-1 rounded-full text-xs font-semibold ${getTypeBadge(selectedLead.lead_type).cls}`}>
                      {selectedLead.lead_type}
                    </span>
                    {selectedLead.auction_type && (
                      <span className={`inline-block px-3 py-1 rounded-full text-xs font-semibold ${
                        getLeadTypeNormalized(selectedLead.auction_type) === 'FORECLOSURE' ? 'bg-orange-500/20 text-orange-400' : 'bg-purple-500/20 text-purple-400'
                      }`}>
                        {selectedLead.auction_type}
                      </span>
                    )}
                  </div>
                  <h1 className="text-3xl font-bold text-white mb-2">{selectedLead.property_address || 'No Address'}</h1>
            ...
            <div className="bg-slate-800/50 border border-slate-700/50 rounded-xl p-6">
              <h2 className="text-xl font-bold text-white mb-4 flex items-center gap-2"><UsersIcon className="w-5 h-5 text-amber-400" /> Case Parties</h2>
              ...
              ) : <p className="text-slate-500 text-sm">No defendants on record</p>}
            ...
            <div className="bg-slate-800/50 border border-slate-700/50 rounded-xl p-6">
              <h2 className="text-xl font-bold text-white mb-4 flex items-center gap-2"><MapPin className="w-5 h-5 text-amber-400" /> Property Details</h2>
              <div className="mb-4 p-3 bg-slate-900/50 rounded-lg">
                <p className="text-slate-400 text-sm mb-1">Property Address</p>
                <p className="text-white font-semibold text-lg">{selectedLead.property_address || 'N/A'}</p>
                {selectedLead.property_city && (
                  <p className="text-slate-300 text-sm">{selectedLead.property_city}{selectedLead.property_zip && `, ${selectedLead.property_zip}`}</p>
                )}
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                ...
                <div className="p-3 bg-slate-900/50 rounded-lg">
                  <p className="text-slate-400 text-sm mb-1">{getLeadTypeNormalized(selectedLead.auction_type) === 'TAXDEED' || getLeadTypeNormalized(selectedLead.auction_type) === 'TAX DEED' ? 'Opening Bid' : 'Judgment Amount'}</p>
              ...
              {/* ── ALL LINKS FROM DETAIL PAGE ── */}
              <div className="flex flex-wrap gap-3">
                {selectedLead.case_url && (
                  <a href={selectedLead.case_url} target="_blank" rel="noopener noreferrer" className="px-4 py-2 bg-slate-700 hover:bg-slate-600 text-white rounded-lg text-sm font-medium">View Case →</a>
                )}
                {selectedLead.final_judgment_url && (
                  <a href={selectedLead.final_judgment_url} target="_blank" rel="noopener noreferrer" className="px-4 py-2 bg-slate-700 hover:bg-slate-600 text-white rounded-lg text-sm font-medium">Final Judgment →</a>
                )}
                {selectedLead.clerk_of_courts_url && (
                  <a href={selectedLead.clerk_of_courts_url} target="_blank" rel="noopener noreferrer" className="px-4 py-2 bg-slate-700 hover:bg-slate-600 text-white rounded-lg text-sm font-medium">Clerk of Courts →</a>
                )}
                {selectedLead.property_appraiser_url && (
                  <a href={selectedLead.property_appraiser_url} target="_blank" rel="noopener noreferrer" className="px-4 py-2 bg-slate-700 hover:bg-slate-600 text-white rounded-lg text-sm font-medium">Property Appraiser →</a>
                )}
                {selectedLead.street_map_url && (
                  <a href={selectedLead.street_map_url} target="_blank" rel="noopener noreferrer" className="px-4 py-2 bg-slate-700 hover:bg-slate-600 text-white rounded-lg text-sm font-medium">Street Map →</a>
                )}
                {selectedLead.satellite_url && (
                  <a href={selectedLead.satellite_url} target="_blank" rel="noopener noreferrer" className="px-4 py-2 bg-slate-700 hover:bg-slate-600 text-white rounded-lg text-sm font-medium">Satellite View →</a>
                )}
                {selectedLead.zillow_url && (
                  <a href={selectedLead.zillow_url} target="_blank" rel="noopener noreferrer" className="px-4 py-2 bg-slate-700 hover:bg-slate-600 text-white rounded-lg text-sm font-medium">Zillow →</a>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-slate-800/50 border border-slate-700/50 rounded-xl p-6">
              <h3 className="text-lg font-bold text-white mb-4 flex items-center gap-2"><User className="w-5 h-5 text-amber-400" /> Contact Information</h3>
              <SimpleContactForm leadId={selectedLead.id} contacts={leadContacts} onSave={saveContact} onDelete={deleteContact} />
            </div>
      ...
      <div className="bg-slate-800/50 border-b border-slate-700/50 sticky top-0 z-50 px-6 py-4">
        <div className="flex items-center justify-between max-w-7xl mx-auto">
          ...
          <div className="flex items-center gap-4">
            {user?.role === 'admin' && (
              <>
                <button onClick={() => setView('admin')} className="px-4 py-2 bg-slate-700 hover:bg-slate-600 text-white rounded-lg flex items-center gap-2"><UsersIcon className="w-4 h-4" /> Admin</button>
                <button onClick={exportData} className="px-4 py-2 bg-slate-700 hover:bg-slate-600 text-white rounded-lg flex items-center gap-2"><Download className="w-4 h-4" /> Export</button>
              </>
            )}
            <div className="flex items-center gap-3 px-4 py-2 bg-slate-700/50 rounded-lg">
              <span className="text-white font-medium">{user?.name}</span>
              <span className={`px-2 py-1 rounded text-xs font-semibold ${user?.role === 'admin' ? 'bg-amber-500/20 text-amber-400' : 'bg-blue-500/20 text-blue-400'}`}>{user?.role?.toUpperCase()}</span>
            </div>
            <button onClick={logout} className="p-2 bg-slate-700 hover:bg-slate-600 text-white rounded-lg"><LogOut className="w-5 h-5" /></button>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
          <div className="bg-slate-800/50 border border-slate-700/50 rounded-xl p-4">
            <div className="flex items-center justify-between mb-2"><span className="text-slate-400 text-sm">Total</span><FileText className="w-4 h-4 text-amber-400" /></div>
            <p className="text-3xl font-bold text-white">{stats.total.toLocaleString()}</p>
          </div>
          <div className="bg-blue-500/10 border border-blue-500/30 rounded-xl p-4">
            <div className="flex items-center justify-between mb-2"><span className="text-slate-400 text-sm">New</span><Plus className="w-4 h-4 text-blue-400" /></div>
            <p className="text-3xl font-bold text-blue-400">{stats.new.toLocaleString()}</p>
          </div>
          <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-4">
            <div className="flex items-center justify-between mb-2"><span className="text-slate-400 text-sm">Contacted</span><Calendar className="w-4 h-4 text-amber-400" /></div>
            <p className="text-3xl font-bold text-amber-400">{stats.contacted.toLocaleString()}</p>
          </div>
          <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-xl p-4">
            <div className="flex items-center justify-between mb-2"><span className="text-slate-400 text-sm">Interested</span><TrendingUp className="w-4 h-4 text-emerald-400" /></div>
            <p className="text-3xl font-bold text-emerald-400">{stats.interested.toLocaleString()}</p>
          </div>
          <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-xl p-4">
            <div className="flex items-center justify-between mb-2"><span className="text-slate-400 text-sm">Surplus</span><DollarSign className="w-4 h-4 text-emerald-400" /></div>
            <p className="text-3xl font-bold text-emerald-400">{stats.surplus.toLocaleString()}</p>
          </div>
          <div className="bg-blue-500/10 border border-blue-500/30 rounded-xl p-4">
            <div className="flex items-center justify-between mb-2"><span className="text-slate-400 text-sm">Future</span><Calendar className="w-4 h-4 text-blue-400" /></div>
            <p className="text-3xl font-bold text-blue-400">{stats.future.toLocaleString()}</p>
          </div>
          <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-xl p-3 col-span-2 md:col-span-3 lg:col-span-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-emerald-400">${stats.totalSurplus.toLocaleString('en-US', { maximumFractionDigits: 0 })}</p>
              </div>
        ...
        <div className="flex items-center gap-2 mb-4">
          ...
          <button onClick={() => { if (sortBy === 'date') setSortOrder(sortOrder === 'desc' ? 'asc' : 'desc'); else { setSortBy('date'); setSortOrder('desc') } }} className={`px-3 py-1 rounded-lg text-sm flex items-center gap-1 ${sortBy === 'date' ? 'bg-amber-500 text-white' : 'bg-slate-700 text-slate-300'}`}>
            Date {sortBy === 'date' && (sortOrder === 'desc' ? '↓' : '↑')}
          </button>
          <button onClick={() => { if (sortBy === 'surplus') setSortOrder(sortOrder === 'desc' ? 'asc' : 'desc'); else { setSortBy('surplus'); setSortOrder('desc') } }} className={`px-3 py-1 rounded-lg text-sm flex items-center gap-1 ${sortBy === 'surplus' ? 'bg-amber-500 text-white' : 'bg-slate-700 text-slate-300'}`}>
        ...
        {selectedLeads.length > 0 && (
          <div className="bg-slate-700/50 border border-slate-600 rounded-xl p-4 mb-4 flex items-center justify-between">
            ...
            <div className="flex gap-2">
              {user?.role === 'admin' && (
                <>
                  <select onChange={(e) => { if (e.target.value) { selectedLeads.forEach(id => assign(id, e.target.value)); setSelectedLeads([]) } }} className="px-4 py-2 bg-slate-800 border border-slate-600 text-white rounded-lg">
                    <option value="">Assign to...</option>
                    {users.filter(u => u.role === 'user').map(u => <option key={u.id} value={u.id}>{u.name}</option>)}
                  </select>
                  <button onClick={() => { if (window.confirm(`Delete ${selectedLeads.length} leads?`)) { selectedLeads.forEach(id => deleteLead(id)); setSelectedLeads([]) } }} className="px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg">Delete Selected</button>
                </>
              )}
              <button onClick={() => setSelectedLeads([])} className="px-4 py-2 bg-slate-600 hover:bg-slate-500 text-white rounded-lg">Clear</button>
            </div>
          </div>
        )}
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-slate-900/50 border-b border-slate-700/50">
                <th className="text-left px-2 py-2 text-xs font-semibold text-slate-300" style={{width:'40px'}}>
                  <input type="checkbox" checked={sortedFiltered.length > 0 && selectedLeads.length === sortedFiltered.length} onChange={(e) => { if (e.target.checked) setSelectedLeads(sortedFiltered.map(l => l.id)); else setSelectedLeads([]) }} className="w-4 h-4 rounded border-slate-600 bg-slate-700" />
                </th>
            ...
            <tbody>
              {sortedFiltered.map((l, i) => {
                const badge = getTypeBadge(l.lead_type)
                const surplusData = formatSurplus(l.surplus)
                return (
                  <tr key={l.id} className={`border-b border-slate-700/30 hover:bg-slate-700/30 ${i % 2 === 0 ? 'bg-slate-900/20' : ''}`}>
                    <td className="px-2 py-2" style={{width:'40px'}}>
                      <input type="checkbox" checked={selectedLeads.includes(l.id)} onChange={(e) => { if (e.target.checked) setSelectedLeads([...selectedLeads, l.id]); else setSelectedLeads(selectedLeads.filter(id => id !== l.id)) }} className="w-4 h-4 rounded border-slate-600 bg-slate-700" />
                    </td>
                    ...
                    <td className="px-2 py-2" style={{width:'60px'}}>
                      <span className={`px-1 py-0.5 rounded text-xs font-semibold block text-center ${badge.cls}`}>{badge.label}</span>
                    </td>
                    ...
                    <td className="px-2 py-2 font-semibold text-xs truncate" style={{width:'90px'}}>
                      {surplusData ? (
                        <span className={surplusData.negative ? 'text-red-400' : 'text-emerald-400'}>
                          {surplusData.negative ? '-' : ''}{surplusData.formatted}
                        </span>
                      ) : '—'}
                    </td>
                    ...
                    <td className="px-2 py-2" style={{width:'120px'}}>
                      <div className="flex items-center gap-1">
                        <button onClick={() => viewLead(l)} className="flex items-center gap-1 px-2 py-1 bg-slate-700 hover:bg-slate-600 text-white rounded text-xs"><Eye className="w-3 h-3" /> View</button>
                        {user?.role === 'admin' && (
                          <button onClick={() => { if (window.confirm(`Delete ${l.case_number}?`)) deleteLead(l.id) }} className="px-2 py-1 bg-red-600 hover:bg-red-700 text-white rounded text-xs">Del</button>
                        )}
                      </div>
                    </td>
                  </tr>
